    ...
    const getRecipe = async () => {
      try {
        const { data } = await axios.get(
          `${process.env.REACT_APP_BASE_URL}:${process.env.REACT_APP_SERVER_PORT}/api/recipes/${clickedID}`
        );
        console.log(data);
  ...
  const handleDelete = () => {
    const sendDeleteRecipe = async () => {
      return await axios.delete(
        `${process.env.REACT_APP_BASE_URL}:${process.env.REACT_APP_SERVER_PORT}/api/recipes/${currentRecipeID}/delete`
      );
    };
    sendDeleteRecipe();
    return setTimeout(() => {
