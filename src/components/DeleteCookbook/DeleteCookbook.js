    ...
    const getCookbook = async () => {
      try {
        const { data } = await axios.get(
          `${process.env.REACT_APP_BASE_URL}:${process.env.REACT_APP_SERVER_PORT}/api/cookbooks/${clickedID}`
        );
  ...
  const handleDelete = () => {
    const sendDeleteCookbook = async () => {
      return await axios.delete(
        `${process.env.REACT_APP_BASE_URL}:${process.env.REACT_APP_SERVER_PORT}/api/cookbooks/${currentCookbookID}/delete`
      );
    };
    sendDeleteCookbook();
